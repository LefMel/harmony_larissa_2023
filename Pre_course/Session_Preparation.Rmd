---
title: "Pre-course material - HARMONY - Grant Period 4 - Larissa, Greece Training School 25 - 27 October 2023"
author:
- Eleftherios Meletis, Konstantinos Pateras, Xanthi Rousou, Polychronis Kostoulas, Matthew Denwood
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
This document will help you to prepare for the advanced training school on Bayesian methods being held in Larissa, Greece, 25 - 27 October 2023. The training school involves both introductory didactic teaching and hands-on practical sessions on Bayesian methods for true prevalence estimation & diagnostic test evaluation. The following core topics will be covered:

* Bayesian Markov Chain Monte Carlo (MCMC)
* Apparent & true prevalence estimation
* Test evaluation without cut-offs
* Test evaluation with Hui-Walter models

Primary learning objectives of the school are that participants (i) become familiar with methods for true prevalence estimation and diagnostic test evaluation (ii) understand the basic principles of MCMC and where Bayesian Latent class models can be applied.

# Software-Package installation

You need to install R (version 4.2.0 or later) from https://cran.r-project.org/ and we recommend that you also use Rstudio which can be downloaded separately from https://www.rstudio.com/products/rstudio/download/
...
Although it is not essential, we also recommend that you update to the latest stable release of the runjags package:

```{r eval=FALSE}
install.packages("runjags", repos=c(CRAN = "https://cran.rstudio.com/", 
...
In case you have a previous R version (4.1.x) installed you need to match your R and JAGS versions as follows:
...
We have created a public repository containing the teaching material for the training workshop. This means that anyone can view the teaching material at any time via the following website: https://github.com/LefMel/harmony_larissa_2023

You should see a number of folders listed with names like "Pre_course", "Day1" etc, and within each of these folders you will find material for each of the training school sessions. For each session there will be two files:  a .pdf file showing just the presentation that we will give for the didactic session, and a .html file that also includes the exercises and solutions for these exercises. You can click on any of these files to view them, although of the different file formats the .pdf version is probably easiest to download directly from the website. There are two problems with this:
...
The first step is to clone the `harmony_larissa_2023` repository on to your computer. From inside GitHub Desktop, go to the File menu then Clone Respository, then click on the URL tab.  Then type/paste `LefMel/harmony_larissa_2023` into the box where it says `Filter your reposotories` or `Repository URL` or `GitHub username and repository` (depending on your version of GitHub Desktop). A suggested local path folder will be created automatically at the bottom but feel free to change this. Then click on Clone and wait for the files to download.

Creating the clone copies all of the files currently on the GitHub repository to your computer at the local path specified above (if you can't remember what this was choose 'Show in Windows/Finder' or similar, depending on your OS, under the Respository menu). This is where all of the course material will be located. For example, look inside the `'Pre_course'` folder and open up the `'Session_Preparation.html'` file - that is a local copy of the same document you are currently reading! But now you also have the PDF version - use this if you want to print the document for some reason, but it is a good idea to stick to using the HTML version if you want to copy/paste R code (you will probably encounter problems with quotation marks if you copy/paste R code from PDF files).  The two versions should be identical.
...
The easiest way to clone the `harmony_larissa_2023` repository on to your computer is via RStudio. Go to the `File` menu and select `New project`. It will open and wizard menu, giving you three alternative ways to create a project: select `Version Control` and then `GIT`. Then type/paste `https://github.com/LefMel/harmony_larissa_2023.git` into the box where it says `Repository URL`. A suggested local path folder will be created automatically at the bottom but feel free to change this. Then click on `Clone` and wait for the files to download.
...
For example, look inside the `'Pre_course'` folder and open up the `'Session_Preparation.html'` file - that is a local copy of the same document you are currently reading! But now you also have the PDF version - use this if you want to print the document for some reason, but it is a good idea to stick to using the HTML version if you want to copy/paste R code (you will probably encounter problems with quotation marks if you copy/paste R code from PDF files). 
